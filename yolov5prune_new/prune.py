...
def run(data,
    ...
    # Dataloader
    if not training:
        model.warmup(imgsz=(1, 3, imgsz[0], imgsz[1]), half=half)  # warmup
  ...
  #  print("prune module :",model_list.keys())
   ...
   # print(maskbndict.keys())
    ...
    for ((layername, layer),(pruned_layername, pruned_layer)) in zip(model.named_modules(), pruned_model.named_modules()):
        # print(f"Layer name: {layername}, Layer: {layer}")
        # print(f"Pruned layer name: {pruned_layername}, Pruned layer: {pruned_layer}")
        # print('-' * 50)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/my_yolov5_7.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train_sparity/exp27/weights/last.pt', help='model.pt path(s)')
    # parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'pruned_model_new3.pt', help='model.pt path(s)')
    parser.add_argument('--cfg', type=str, default='models/yolov5n.yaml', help='model.yaml path')
    parser.add_argument('--percent', type=float, default=0.2, help='prune percentage')
    parser.add_argument('--batch-size', type=int, default=2, help='batch size')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=[640, 360], help='inference size (pixels)')
